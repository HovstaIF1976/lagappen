...
Deno.serve(async (req) => {
  ...
  try {
    const body = (await req.json()) as PlayerLoginRequest
    ...
    if (signInError || !signInData.session || !signInData.user) {
      console.error(
        "Player Auth sign-in failed:",
        signInError,
      )

      return Response.json(
        { error: "Kunde inte logga in spelaren." },
        {
          status: 500,
          headers: corsHeaders,
        },
    ...
    console.error("Unexpected player-login error:", error)
